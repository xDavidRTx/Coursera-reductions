...
object LineOfSight {
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if(size > threshold){
      val half = size/2
      val (t1, t2) = parallel(upsweep(input, from, from+half, threshold),upsweep(input, from+half, end, threshold) )
      Node(t1,t2)
    }else{
